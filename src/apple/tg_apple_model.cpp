...
{
	m_winTarget = std::clamp(value, 100, 900);
}
...
void TgAppleModel::spawnAppleIfNeeded()
{
	...
	if (nullptr == p)
		return;
	const int gear = randomInt(0, 9);
	const qreal base = 60.0 + static_cast<qreal>(m_level) * 28.0;
	p->m_speedY = base + static_cast<qreal>(gear) * 12.0;
