...
bool runAppleSettings(TgConfig *pConfig, QWidget *pParent)
{
	...
	public:
			...
			, m_checkSprite(m_rp.spacePixmap(QStringLiteral("CHECKBOX_BUTTON")))
		{
			...
			m_pLevel = new QSlider(Qt::Horizontal, this);
			m_pFail = new QSlider(Qt::Horizontal, this);
			m_pWin = new QSlider(Qt::Horizontal, this);
			m_pMax = new QSlider(Qt::Horizontal, this);
			...
			applySliderSkin(m_pLevel);
			applySliderSkin(m_pFail);
			applySliderSkin(m_pWin);
			applySliderSkin(m_pMax);
	...
	private:
		void buildFixedLayout()
		{
			...
			// Label positions (reference): (120,80/140/200), values at (400,80/140/200)
			applyLabel(m_pLevelLabel, QString::number(m_pLevel->value()), 420, 80, true);
			applyLabel(m_pFailLabel, QString::number(m_pFail->value()), 420, 140, true);
			applyLabel(m_pWinLabel, QString::number(m_pWin->value()), 420, 200, true);
			applyLabel(m_pMaxLabel, QString::number(m_pMax->value()), 420, 260, true);
			...
			applyLabel(lblLevel, QString::fromUtf16(u"\u6e38\u620f\u7b49\u7ea7:"), 120, 80, false);
			applyLabel(lblFail, QString::fromUtf16(u"\u5931\u8d25\u82f9\u679c\u6570:"), 120, 140, false);
			applyLabel(lblWin, QString::fromUtf16(u"\u6210\u529f\u82f9\u679c\u6570:"), 120, 200, false);
			applyLabel(lblMax, QString::fromUtf16(u"\u540c\u5c4f\u82f9\u679c:"), 120, 260, false);
			applyLabel(lblSound, QString::fromUtf16(u"\u97f3\u6548:"), 120, 320, false);

			auto applySlider = [&](QSlider *slider, int x, int y) {
				slider->setFixedWidth(static_cast<int>(200 * s));
				slider->move(static_cast<int>(x * s), static_cast<int>(y * s));
				applySliderSkin(slider);
			};
			// Slider positions (reference): (200,80/140/200)
			applySlider(m_pLevel, 200, 80);
			applySlider(m_pFail, 200, 140);
			applySlider(m_pWin, 200, 200);
			applySlider(m_pMax, 200, 260);

			m_pSound->setFixedSize(m_pSound->sizeHint());
			m_pSound->move(static_cast<int>(200 * s), static_cast<int>(316 * s));
		...
		void applySliderSkin(QSlider *pSlider)
		{
			...
			const QString grooveUrl = QUrl::fromLocalFile(groovePath).toString();
			const QString handleUrl = QUrl::fromLocalFile(handlePath).toString();

			const QString qss = QStringLiteral(R"(
QSlider::groove:horizontal {
	border-image: url(%1) 0 1 0 1 repeat stretch;
	height: 4px;
	border: none;
}
...
void TgAppleGameWidget::layoutOverlay()
{
	...
	auto applyScaled = [&](TgImageButton *btn, int x, int y) {
		...
		const QSize scaled(std::max(1, static_cast<int>(raw.width() * s)),
			std::max(1, static_cast<int>(raw.height() * s)));
		btn->setFixedSize(scaled);
		btn->move(static_cast<int>(x * sx), static_cast<int>(y * sy));
	};

	applyScaled(m_pPauseBtn, 118, 514);
	applyScaled(m_pStartBtn, 198, 526);
	applyScaled(m_pEndBtn, 163, 483);
	applyScaled(m_pSettingsBtn, 150, 552);
