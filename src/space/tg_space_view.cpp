...
void TgSpaceView::paintEvent(QPaintEvent* pEvent)
{
    ...
    const QPixmap plane = m_pResources->spacePixmap("SPACE_ENEMY_0");
    const QPixmap meteor = m_pResources->spacePixmap("SPACE_ENEMY_4");
    const QPixmap bomb = m_pResources->spacePixmap("SPACE_BOMB");
    const QPixmap boom = m_pResources->spacePixmap("SPACE_EXPLOSION_0");
    ...
    for (const TgSpaceEnemy* e : m_pModel->enemies()) {
        ...
        painter.drawPixmap(er.toAlignedRect(), src.scaled(er.toAlignedRect().size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
        painter.setPen(Qt::white);
