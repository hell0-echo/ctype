...
void TgSpaceView::paintEvent(QPaintEvent* pEvent)
{
    ...
    auto spriteSrcRect = [](const QPixmap& sheet, int cols, int rows, int idx) -> QRect {
    ...
    for (const TgSpaceEnemy* e : m_pModel->enemies()) {
        ...
        if (e->m_explosionFrame >= 0) {
            ...
            if (!boomSheet.isNull()) {
                ...
                const QRect src = spriteSrcRect(boomSheet, 3, 3, f);
        ...
        const QRect srcRect = spriteSrcRect(sheet, 4, 4, frameIdx);
        if (!srcRect.isNull())
    ...
    if (!shipSheet.isNull()) {
        ...
        const QRect srcRect = spriteSrcRect(shipSheet, 4, 4, shipFrame);
        if (!srcRect.isNull())
